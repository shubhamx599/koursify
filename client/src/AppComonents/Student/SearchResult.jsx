...
const SearchResult = ({ course }) => (
  <Link to={`/detail-page/${course._id}`} className="group flex flex-col gap-5 rounded-[24px] border border-white/10 bg-[#0d1d19] p-3 transition hover:border-[#c9ff62]/30 sm:flex-row">
    <div className="aspect-[16/10] w-full overflow-hidden rounded-[18px] bg-[#17352d] sm:w-56 shrink-0 grid place-items-center">
      {course.courseThumbnail ? (
        <img src={course.courseThumbnail} alt={course.courseTitle} className="h-full w-full object-cover transition duration-500 group-hover:scale-105"/>
      ) : (
        <div className="text-3xl font-black text-[#c9ff62]/25">K.</div>
      )}
    </div>
